...
namespace embree
{
  ...
  struct Triangle4iIntersector1Pluecker
  {
    ...
    static __forceinline void intersect(const Precalculations& pre, Ray& ray, size_t ty, void* tri, size_t num, void* geom)
    {
      for (size_t i=0; i<num; i++)
        intersect(pre,ray,((const Triangle4i*)tri)[i],geom);
    }
    ...
    static __forceinline bool occluded(const Precalculations& pre, Ray& ray, size_t ty, void* tri, size_t num, const void* geom) 
    {
      for (size_t i=0; i<num; i++) 
        if (occluded(pre,ray,((const Triangle4i*)tri)[i],geom))
          return true;
